...
class NLL_Trainer(object):
    ...
    def __init__(   self, transmit, model, train_set, valid_set, test_set,
        ...
        #  No reference to the sets should be kept here, only dimensions
        self.train_set = train_set
        self.valid_set = valid_set
        self.test_set  = test_set

        # Any change in performance having those be shared variables?
        ...
        # How, in lasagne, do they do to have update scheles and functions separated in different modules
        train_model_with_weight_drop = theano.function(
            inputs=[self.index],
            outputs=[cost].append(g_params),
            updates=updates,
            givens={
        ...
        train_model_without_weight_drop = theano.function(
            inputs=[self.index],
            outputs=[cost],
            updates=updates,
            givens={
        ...
        self.train_functions = {
            'no_grad'   : train_model_without_weight_drop,
            'grad'      : train_model_with_weight_drop
        }
        self.train_model = self.train_functions[self.state.grad_state]
        ...
        while (epoch < self.n_epochs) and self.is_running:
            ...
            for minibatch_index in xrange(self.n_train_batches):                            # minibatch_index within an epoch
                ...
                if (iter + 1) % self.record_frequency == 0 and self.is_recording:
                    ...
                    if self.state.grad_state == 'grad':
                        # How do we index gradients in the queue
                        # If we have special functions for each weight dropping state
                        # How do we handle it?
                        data['grad'] = res[1]
                    if data:
                ...
                if (iter + 1) % self.validation_frequency == 0:
                    ...
                    minibatch_avg_cost = 0
                    this_validation_loss = timer.time(self.validate_model)
                    self.log.info(
    ...
    def add_target(self, target, layers):
        '''
            Set the target layer as to be recorded. See model.target method
        '''
        if target[3] is not None:
            return all(self.state.add_target(layer) for layer in layers)
        else:
            return all(self.model.add_target(layer) for layer in layers)
    ...
    def remove_target(self, target = None):
        ...
        '''
        if target[3] is not None:
            return self.state.remove_target(target)
        else:
