...
def fast_init_image(dico):
    ...
    background_ret = []
    ax_ret = []
    log.debug('axes = {}'.format(ax))
    for raw in ax:
        if ind >= len(keys):
            log.warn('Odd Breaking while init')
            break
        if type(raw) is not np.ndarray:
            log.debug('type(raw) {}'.format(type(raw)))
            log.debug('raw.shape {}'.format(raw.shape))
            raw = [raw]
            log.debug('only one raw')
        for column in raw:
            log.debug('back in loop')
            log.debug('plot title {}'.format(keys[ind]))
...
def print_image_fast(dico, (fig, ax, backgrounds)):
    log.verb('Printing: {}'.format(dico))
    keys = dico.keys()
    for ind in range(len(keys)):
        log.debug([ax[ind]])
        ...
        log.debug('ax[ind].images[0] : {}'.format(ax[ind].images[0].get_array()))
        # Apparently blit should not be used anymore
        # Use update instead
        fig.canvas.blit(ax[ind].bbox)
        #fig.canvas.update()
        fig.canvas.draw()
        log.debug('Has drawn image')
